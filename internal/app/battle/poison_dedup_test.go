...
// TestActorAppearsBefore: exact identity, only slots strictly before the cursor; a cursor past queue end clamps not panics.
...
// TestIngestedPoisonTicksOncePerRound: an ingested member holding MULTIPLE queue slots (ATB carry-over) ticks poison ONCE that round.
func TestIngestedPoisonTicksOncePerRound(t *testing.T) {
	g := newTestState()
	// Slot 2: poisoned, swallowed, holding TWO slots this round.
	g.Party[2].PoisonTurns = 3
	g.Party[2].HP = 8
	g.Party[2].Ingested = true
	g.Party[2].IngestedBy = 0
	// Two slot-2 entries, then a living actor (slot 0) to break the skip loop before beginNewRound.
	...
	// The tick must deal damage: the ingested-lockout early-return once zeroed it (a free poison escape).
	if g.Party[2].HP >= 8 {
