...
// TestEnemyHitChance_DEXScalesAndClamps checks the accuracy curve: DEX raises
// the hit chance linearly, and the result clamps to [Floor, Cap] so even a
// max-DEX foe can whiff and a debuffed (low/zero DEX) one still lands sometimes.
func TestEnemyHitChance_DEXScalesAndClamps(t *testing.T) {
	...
	// Floor: a zero/negative-DEX (heavily blinded) foe still lands sometimes.
