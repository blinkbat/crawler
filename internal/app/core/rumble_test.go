...
// TestTriggerRumble_KeepStronger: stronger overrides a running buzz, weaker doesn't, clamps to 1, non-positive no-ops.
...
// TestTickRumble_DecaysToZero: envelope decays linearly, clamps level + timer at zero.
func TestTickRumble_DecaysToZero(t *testing.T) {
	...
	// Past the end: clamps to 0, no negative.
	...
	if lvl := TickRumble(&b, 0.1); lvl != 0 { // inactive ticks to 0
