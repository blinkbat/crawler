...
- `internal/app/explore/movement.go`: pause menu input (P / Esc / gamepad Start), tile movement (props + chests + deep water block; doors trigger area transitions on step-land), smooth free-look return, step/turn animations, adjacent encounter + chest checks (`tryOpenAdjacentChest`), `Visited` fog-of-war marking. The Update gate routes level-up modal → Party Stats overlay → game panels overlay → chest modal → pause menu → battle / explore.
...
- `internal/app/render/levelup.go`: post-battle level-up modal (`DrawLevelUpModal`) and the read-only Party Stats overlay (`DrawPartyStatsScreen`) — both read the table-driven `core.StatLabel` / `core.StatValue`. The pause menu's "Party Stats" entry routes here for the legacy compact view; the panels overlay's Stats tab is the richer multi-tab version.
...
- `internal/app/editor/ops.go`: brush apply / erase, undo/redo, multi-tile footprint validation, pack + chest + door edit ops (`placeChestAt`, `placeDoorAt`, `filterChests` / `filterDoors`), resize, reachability warnings, `rewriteLayerRows` row-mutation helper shared by `floodFill` / `fillEntireLayer`, `centerViewOnTile`.
