...
- Mixed initiative: party + enemies are sorted by SPD into a single per-round queue. Both sides read `Stats.SPD` (enemy `EnemyDefinition.Speed` was retired in favor of full `Stats` symmetry). SPD also drives turn FREQUENCY: actors whose SPD exceeds the round's slowest by `SpdBonusTurnDivisor` get bonus turn slots appended (capped at `SpdMaxBonusTurns`). Burn ticks fire at the start of the burning actor's own turn; Poison ticks at the END of the poisoned actor's turn.
...
- Crit (party-side): `rollSkillCrit` rolls per damaging player skill. Universal path uses `core.RollCrit` (`CritBaseline 0.05` + `CritPerDEX 0.015` + `timingGrades.CritBonus`, cap `CritCap 0.6`), multiplies post-crit damage by `core.CritMultiplier = 2`, and appends " Critical!" to the log via `appendCrit`. Backstab keeps its signature "Excellent timing = guaranteed crit" path inside `rollSkillCrit`, and its T2 (`CritDoubleOnExcellent`) stacks an additional ×2 via the helper's `double` return. CrushingBlow T3's separate `CritDoubleOnExcellent` doubles damage on Excellent INDEPENDENTLY of the universal crit (both can stack to ×4). Enemy basic attacks do not crit today.
- Magic Defense: party WIS goes through `core.MagicDefense(s)` and is consumed by `core.ApplyMagicDefense` inside `damagePartyMember` for magic-tagged hits — symmetrical to Armor on the phys side, with the same `max(dmg - mdef, 1)` floor. Enemies have no MDef field yet (default 0); player Firebolt vs enemy is unchanged.
- Status-duration shortening: `core.ShortenStatusDuration(duration, wis)` shaves `wis / StatusShortenDivisor` (=3) turns off enemy-applied Sleep / Poison / Bound / Confuse on a party member (floor 1). A high-WIS Cleric (WIS 6) eats 2 fewer turns of every status; low-WIS Warrior eats the full duration. Confuse still has its prior `WIS / (WIS+k)` apply-resist roll on top.
